...
class Reading(Base):
    ...
    fault_type  = Column(String, nullable=True)        # e.g. "filter_fault", None if healthy
    mic_rms     = Column(Float, nullable=True)         # raw feature values (optional)
    acc_rms     = Column(Float, nullable=True)         # total acc RMS (g)
    acc_x_rms   = Column(Float, nullable=True)         # per-axis acc RMS X (g)
    acc_y_rms   = Column(Float, nullable=True)         # per-axis acc RMS Y (g)
    acc_z_rms   = Column(Float, nullable=True)         # per-axis acc RMS Z (g)
    health_score = Column(Float, nullable=True)        # 0.0 – 1.0 score from model
